...
dts=[ np.power(2, i)/200. for i  in range(3)]
...
for dt in dts:
    ...
    u_history=np.array(u_history)
    state_history=np.array(state_history)
    results[f'{dt}_ro']=(diff_history,u_history,state_history,x_axis)
...
for dt in results:
    ...
    axs[0].plot(x_axis,np.linalg.norm(state_history[:,node_i,2:],axis=1),label=f'velocity norm {node_i} at dt={dt}', alpha=0.9)
...
# plt.plot(x_axis_u4,np.linalg.norm(de_estimate_u1[:,node_i,2:],axis=1),label=f'DE estimate dt = 0.01', alpha=0.7)
# plt.plot(x_axis_u4,np.linalg.norm(de_estimate_u2[:,node_i,2:],axis=1),label=f'DE estimate dt = 0.02', alpha=0.7)
# plt.plot(x_axis_u4,np.linalg.norm(de_estimate_u3[:,node_i,2:],axis=1),label=f'DE estimate dt = 0.04', alpha=0.7)
# plt.legend()
...
# plt.plot(x_axis_u4,np.linalg.norm(u1_downsample[:,node_i,2:],axis=1),label=f'dt = 0.01 ', alpha=0.7)
# plt.legend()
