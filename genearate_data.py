...
min_distance_SRQ_alpha, velocity_diff_max_SRQ_alpha = one_simulation_hw5(noise=0.25)
min_distance_SRQ_beta, velocity_diff_max_SRQ_beta = one_simulation_hw5(noise=-0.25)
...
obs_noise=np.array(obs_noise).flatten() # 
print('obs_noise',np.min(obs_noise),np.max(obs_noise))
for i in range(TIMES):
    #load x_init
    x_init=np.load('./data/x_init.npy')
    min_dist_srq, velocity_diff_max_srq = one_simulation_hw5(noise=0, simulation_noise=obs_noise[i], fix_seed=False, simulation_sec=15, save_fig=False,x_init=x_init)
